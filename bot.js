...
bot.launch({
  ...
  if (e.response && e.response.error_code === 401) {
    console.error('Ошибка 401: Проверьте токен бота в @BotFather. Возможно, он недействителен или отозван.');
  }
  process.exit(1);
});
...
process.on('SIGINT', () => {
  ...
  bot.stop('SIGINT').catch(() => console.log('Бот уже остановлен'));
  process.exit(0);
});
process.on('SIGTERM', () => {
  console.log('Остановка бота...');
  bot.stop('SIGTERM').catch(() => console.log('Бот уже остановлен'));
  process.exit(0);
});